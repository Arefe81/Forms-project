...
interface ExpiredPageProps {
  onRetry: () => void;
  reverse?: boolean; // ← اضافه شد
}

export default function ExpiredPage({ onRetry, reverse = false }: ExpiredPageProps) {
  
  const sendAmount = reverse ? 120 : 100;
  const sendCoin = reverse ? pm : tether;
  const sendLabel = reverse ? "Perfect Money" : "USDT";

  const receiveAmount = reverse ? 100 : 120;
  const receiveCoin = reverse ? tether : pm;
  const receiveLabel = reverse ? "USDT" : "Perfect Money";
      ...
      >
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mt: 5 }}>
          ...
          <Typography sx={{ display: "flex", gap: 1, fontSize: 17, alignItems: "center" }}>
            {sendAmount}
            <img src={sendCoin} width={24} height={24} style={{ borderRadius: reverse ? 13 : 0 }} />
            {sendLabel}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mt: 3 }}>
          ...
          <Typography sx={{ display: "flex", gap: 1, fontSize: 17, alignItems: "center" }}>
            {receiveAmount}
            <img src={receiveCoin} width={24} height={24} style={{ borderRadius: reverse ? 0 : 13 }} />
            {receiveLabel}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 1, mt: 10 }}>
          <svg
            width="40"
            height="40"
            viewBox="0 0 40 40"
